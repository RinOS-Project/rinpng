...
`rpng_decode_rgba`, plus deadline-aware `*_with_deadline` variants. It accepts
static PNG using color types 0, 2, 3, 4, and 6,
...
The `*_with_deadline` variants poll a caller-owned monotonic deadline predicate
while parsing, inflating, filtering, and converting pixels; expiry returns
`RPNG_ERR_DEADLINE` and clears caller output. The `rpng.h` C interface is
source-level and does not publish a
...
| limits | Input is limited to 64 MiB, each dimension to 4096 pixels, inflated raw image data to 256 MiB, and the DEFLATE stream to 1,048,576 blocks. Limit violations fail decoding; deadline-aware variants additionally return `RPNG_ERR_DEADLINE` when the caller predicate expires. |
| errors | Malformed, unsupported, truncated, over-limit, or deadline-expired data returns failure. Output is valid only after successful completion. |
